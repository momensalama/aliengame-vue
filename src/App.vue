...
const uiState = computed(() => store.state.uiState)
const characterChoices = computed(() => store.state.characterChoices)
const character = computed(() => store.state.character)

const questionIndex = computed(() => store.state.questionIndex)
const questions = computed(() => store.state.questions)

function pickCharacter() {
  store.commit('setUiState', 'game')
  store.commit('setCharacter', characterInput.value)
}
function pickAnswer(character) {
  ...
  <div id="app" class="contain">
    <GameStateStart v-if="uiState === 'start'">
      <fieldset>
        ...
        <div v-for="option in characterChoices" :key="option">
          <input type="radio" :id="option" :value="option" v-model="characterInput" />
          <label :for="option">{{ option }}</label>
        </div>
        <button @click="pickCharacter">Start</button>
      </fieldset>
    </GameStateStart>
    <section v-else-if="uiState === 'game'">
      <svg viewBox="0 -180 1628 1180" class="main">
        ...
        <Friend />
        <Score />
        <Artist v-if="character === 'artist'" />
        <Baker v-else-if="character === 'baker'" />
        <Mechanic v-else />
